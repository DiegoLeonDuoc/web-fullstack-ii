...
describe("FormularioRegistro DOM (Jasmine)", () => {
  let saveUserSpy;

  beforeEach(() => {
    saveUserSpy = spyOn(Storage, "saveUser");
  });

  afterEach(() => {
    cleanup();
    saveUserSpy.and.callThrough();
  ...
  it("guarda un usuario valido y navega al login", async () => {
    ...
    await waitFor(() => expect(submitButton.disabled).toBeFalse());
    ...
    await waitFor(() => {
      expect(saveUserSpy).toHaveBeenCalledWith(jasmine.objectContaining({
        rut: '12.345.678-5',
